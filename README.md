...
**实例演示界面**
1. ***演示:什么是西瓜书***
![什么是西瓜书](./figures/demo1.png)
2. ***演示:贝叶斯网络是什么，马尔科夫链又是什么***
![贝叶斯网络是什么，马尔科夫链又是什么](./figures/demo2.png)
...
这些知识库源数据放置在 **project/knowledge_db** 目录下，用户也可以自己存放自己其他的文件。
...
def get_repos(org_name, token, export_dir):
    ...
    if response.status_code == 200:
        ...
        # 检查文件是否存在，如果不存在则创建
        if not os.path.isfile(repositories_path):
...
if __name__ == '__main__':
    ...
    # 配置 export_dir
    export_dir = "readme_db"  # 请替换为实际的目录路径
    # 获取仓库列表
...
默认会把这些readme文件放在同目录database下的readme_db文件。其中这些readme文件含有不少无关信息，即再运行**project/database/text_summary_readme.py文件**可以调用大模型生成每个readme文件的摘要并保存到上述知识库目录../knowledge_db/readme_summary文件夹中，****。代码如下：
...
def generate_llm_summary(repo_name, readme_content,model):
    ...
    llm = ZhipuAI(api_key=zhipuai_api_key)
...
```python
......
def create_db(files=DEFAULT_DB_PATH, persist_directory=DEFAULT_PERSIST_PATH, embeddings="bge-large-zh"):
    """
...
def get_embedding(embedding: str, embedding_key: str=None, env_file: str=None):
    if embedding == 'm3e':
        ...
        return HuggingFaceEmbeddings(model_name="../local_emb_llm/m3e-base")
    elif embedding == 'bge-large-zh':
        return HuggingFaceEmbeddings(model_name="../local_emb_llm/bge-large-zh")
...
```python
def load_create_test():
    persist_directory = '../vector_db/chroma'
    embeddings = get_embedding(embedding="bge-large-zh")
