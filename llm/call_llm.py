...
def get_completion(prompt :str, model :str, temperature=0.1,api_key=None, secret_key=None, access_token=None, appid=None, api_secret=None, max_tokens=2048):
    ...
    if model == "Qwen1.5-0.5B-Chat":
        ...
        return get_completion_gpt_local(prompt, llm, temperature, api_key, max_tokens)
...
def get_completion_gpt_local(prompt : str, model : str, temperature : float, api_key:str, max_tokens:int):
    default_template_rq = """使用以下上下文来回答最后的问题。如果你不知道答案，就说你不知道，不要试图编造答
        案。最多使用三句话。尽量使答案简明扼要。总是在回答的最后说“谢谢你的提问！”。
        {context}
        问题: {question}
        有用的回答:"""
    chain = LLMChain(llm=model, prompt=default_template_rq)
